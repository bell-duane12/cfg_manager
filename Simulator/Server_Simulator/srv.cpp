...
void MqttServerWrapper::mountHeader(HeaderMessage &h, MessageType t, uint64_t serial, uint64_t reqId)
{
    ...
    /*
        ...
        {
            required ProtocolID protocol		= 1; // protocol ID
            required MessageType msgType		= 2; // message type
            required uint64 serial		= 3; // device serial
            ...
            if (notification.has_header())
            {
                ...
                if (notification.has_eqpstatus())
                {
                    ...
                    if (notification.eqpstatus().has_timestamp() &&
                        notification.eqpstatus().has_online())
                    { // event
...
bool MqttServerWrapper::keepAlive() {
    ...
    message UploadToGateway {
      ...
      optional CheckGateway checkGw;
    }
    */

    UploadToGateway upload;                 // Mensagem a ser enviada para o GW

    HeaderMessage *h = new HeaderMessage;   // Header da mensagem a ser enviada para o GW
    CheckGateway *c = new CheckGateway;     // Status do GW

    mountHeader(*h, MSG_TYPE_USER, 0, 12345); // Monta o header, onde serial = 0 e reqId = 12345
